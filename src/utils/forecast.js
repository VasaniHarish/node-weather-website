...
const forecast = (longitude,latitude, callback) => {
  ...
  request({ url, json: true }, (error, {body}) => {
    if (error) {
      ...
      callback(undefined, 'Right now it is ' + body.current.weather_descriptions[0] + '.' + '\r\n' + ' It is currently ' + ((body.current.temperature-32)*5/9).toFixed(2) + ' degress out, but feels like ' + ((body.current.feelslike-32)*5/9).toFixed(2) + ' degress.' + ' There is a ' + body.current.precip + '% chance of rain. Plan your day accordingly!!')
    }
  })
}
